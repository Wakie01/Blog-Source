...
- 如果是基本类型的包装类型，如Boolean、Character、Byte、Shot、Integer、Double、Float、String、Long，表示判断对象内容是否相同；
- 如果是其他对象的方法，比较的也是引用的内存地址值；
...
> hashCode()方法：默认返回对象在hash表中的地址，其中，hash表中的地址存储着对象存放在内存中的地址。
...
> - NIO（同步非阻塞）：线程发起 IO 请求，立即返回；内核在做好 IO 操作的准备之后，通过调用注册的回调函数通知线程做 IO 操作，线程开始阻塞，直到操作完成。
> - AIO（异步）：线程发起 IO 请求，立即返回；内存做好 IO 操作的准备之后，做 IO 操作，直到操作完成或者失败，通过调用注册的回调函数通知线程做 IO 操作完成或者失败。
